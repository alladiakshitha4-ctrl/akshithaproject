"""
modules/email_detector.py - Email Phishing Detection
"""
...
def show_email_detector():
    ...
    # Load template if button was clicked previously
    default_sender = st.session_state.pop("_tpl_sender", "")
    default_subject = st.session_state.pop("_tpl_subject", "")
    default_body = st.session_state.pop("_tpl_body", "")

    col1, col2 = st.columns([1.5, 1])

    with col1:
        st.markdown("**📝 Email Details**")
        sender = st.text_input("Sender Email", value=default_sender, placeholder="noreply@paypal-secure.tk", key="email_sender")
        subject = st.text_input("Email Subject", value=default_subject, placeholder="URGENT: Verify your account immediately!", key="email_subject")
        body = st.text_area("Email Body", value=default_body, height=250,
    ...
    with col2:
        ...
        indicators_info = [
            ...
            ("🟠", "Dear Customer",             "Impersonal greeting"),
    ...
    <div style='background:{bg}; border:1px solid {border}; border-radius:12px; padding:20px; margin-top:20px;'>
        <div style='display:flex; justify-content:space-between; align-items:center;'>
            <div>
                ...
                <div style='font-size:1.8em; font-weight:800; color:{color};'>{icon} {verdict}</div>
            </div>
        ...
        <div style='background:rgba(0,0,0,0.3); border-radius:6px; height:10px; margin-top:16px; overflow:hidden;'>
            <div style='height:100%; width:{risk}%; background:{color}; border-radius:6px;'></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
