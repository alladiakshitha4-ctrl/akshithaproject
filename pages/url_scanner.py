...
def show_url_scanner():
    ...
    <p style='color: #7eb8d4; margin-bottom: 24px;'>Analyze any URL using AI and extract 22+ security features to detect phishing threats.</p>
    ...
    if scan_clicked and url_input:
        with st.spinner("🤖 AI analyzing URL..."):
            ...
            save_url_scan(user["id"], url_input, result["risk_score"], result["verdict"],
                          result["features"])
...
def _display_scan_result(result, ti, user):
    ...
    # Color mapping
    color_map = {
        "Phishing": ("#ff3860", "badge-phishing", "risk-critical", "🚨"),
        "Suspicious": ("#ff9f43", "badge-suspicious", "risk-high", "⚠️"),
        "Moderate Risk": ("#ffd32a", "badge-moderate", "risk-medium", "🟡"),
        "Safe": ("#00e676", "badge-safe", "risk-low", "✅"),
    }
    color, badge_cls, meter_cls, icon = color_map.get(verdict, ("#aaa", "", "risk-low", "❓"))

    # Main verdict card
    st.markdown(f"""
    <div class='phish-card' style='border-color: {color}40; box-shadow: 0 4px 24px {color}20;'>
        <div style='display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px;'>
            <div style='flex: 1; min-width: 200px;'>
                ...
                <div style='font-size: 2.2em; font-weight: 800; color: {color}; font-family: Exo 2;'>{icon} {verdict}</div>
            ...
            <div style='text-align: center; min-width: 120px;'>
                <div style='font-size: 3.5em; font-weight: 900; color: {color}; font-family: Share Tech Mono, monospace; line-height: 1;'>{risk:.0f}%</div>
    ...
    with col2:
        ...
        for label, val, is_risky in feature_display:
            color_f = "#ff8fa0" if is_risky else "#80ffc0"
            icon_f = "⚠" if is_risky else "✓"
            st.markdown(f"""
            <div style='display: flex; justify-content: space-between; padding: 4px 8px;
                        border-bottom: 1px solid rgba(255,255,255,0.04); font-size: 0.82em;'>
                <span style='color: #7eb8d4;'>{label}</span>
                <span style='color: {color_f}; font-weight: 700;'>{icon_f} {val}</span>
    ...
    with col3:
        ...
        if dom_age.get("risk_note"):
            st.markdown(f"<div style='font-size: 0.75em; color: #7eb8d4; padding: 4px 0; border-top: 1px solid rgba(255,255,255,0.05); margin-top: 6px;'>{dom_age['risk_note']}</div>", unsafe_allow_html=True)
    ...
    with col_prev:
        ...
        if verdict in ("Phishing", "Suspicious"):
            ...
            <div style='background: rgba(255,56,96,0.08); border: 1px solid rgba(255,56,96,0.3);
                ...
                🚫 <strong>Preview Blocked</strong> — Loading this URL could expose your system to threats.<br>
                <span style='font-size: 0.85em; color: #7eb8d4; margin-top: 4px; display: block;'>
                    PhishGuard AI blocked automatic preview to protect you. URL is flagged as {verdict}.
                </span>
            </div>
            """.format(verdict=verdict), unsafe_allow_html=True)
        else:
            url_encoded = result['url'].replace("'", "%27")
            st.markdown(f"""
    ...
    with col_export:
        ...
        if st.button("📥 Download PDF Report", use_container_width=True):
            ...
            fname = f"phishguard_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
