...
def show_statistics():
    ...
    metrics = [
        ...
        ("Detection Rate", f"{detection_rate:.1f}%", "#ff9f43", "🎯"),
        ("Avg Risk Score", f"{avg_risk:.1f}%", "#7eb8d4", "📈"),
    ]
    for col, (label, val, color, icon) in zip(cols, metrics):
        with col:
            st.markdown(f"""
    ...
    with col1:
        ...
        if values_filtered:
            ...
            fig.update_layout(
                title=dict(text="Verdict Distribution", font=dict(color="#00d4aa", size=14, family="Exo 2"), x=0.5),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                font=dict(color="#7eb8d4", family="Exo 2"),
                legend=dict(font=dict(color="#7eb8d4"), bgcolor="rgba(0,0,0,0)"),
                margin=dict(t=40, b=10, l=10, r=10),
                annotations=[dict(text=f"<b>{total}</b><br>Total", x=0.5, y=0.5,
                                  font=dict(size=14, color="#00d4aa", family="Exo 2"), showarrow=False)],
            )
    ...
    with col2:
        ...
        fig.update_layout(
            title=dict(text="Risk Score Distribution", font=dict(color="#00d4aa", size=14, family="Exo 2"), x=0.5),
            paper_bgcolor="rgba(0,0,0,0)",
    ...
    with col3:
        ...
        if "scanned_at" in df.columns and len(df) > 1:
            ...
            fig = go.Figure()
            for verdict, color in [("Phishing", "#ff3860"), ("Suspicious", "#ff9f43"),
                                    ("Moderate Risk", "#ffd32a"), ("Safe", "#00e676")]:
                ...
                if not d.empty:
                    fig.add_trace(go.Scatter(
                        ...
                        fill="tozeroy",
                        fillcolor=color.replace("#", "rgba(") + ",0.08)".replace("rgba(", "rgba(") if "#" in color else color,
                        mode="lines+markers",
                        hovertemplate=f"<b>{verdict}</b><br>Date: %{{x}}<br>Count: %{{y}}<extra></extra>",
                    ))

            fig.update_layout(
                title=dict(text="Scan Activity Timeline", font=dict(color="#00d4aa", size=14, family="Exo 2"), x=0.5),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(13,33,55,0.5)",
                font=dict(color="#7eb8d4", family="Exo 2"),
    ...
    with col4:
        ...
        feature_risks = {
            "IP Address Used": 92, "Brand Squatting": 88, "Suspicious TLD": 85,
            "Phishing Keywords": 78, "URL Shortener": 72, "Redirect Chain": 68,
            "No HTTPS": 60, "High Entropy": 55, "Long URL": 40, "Excess Hyphens": 35,
        }
        ...
        fig.update_layout(
            title=dict(text="Feature Risk Correlation", font=dict(color="#00d4aa", size=14, family="Exo 2"), x=0.5),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(13,33,55,0.5)",
            font=dict(color="#7eb8d4", family="Exo 2"),
    ...
    st.markdown("---")
    st.markdown("""
    <h3 style='color: #00d4aa;'>🤖 AI Model Accuracy Report</h3>
    """, unsafe_allow_html=True)
